...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (e.code === 'P2002') return res.status(409).json({ message: 'Ya existe una Obra Social con ese nombre' });
    res.status(500).json({ message: 'No se pudo crear' });
  }
});

/** PUT /api/social-works/:id  { name } */
router.put('/:id', async (req, res) => {
  const id = String(req.params.id);
  const name = String(req.body?.name || '').trim();
  if (!name) return res.status(400).json({ message: 'El nombre es obligatorio' });
  try {
    const updated = await prisma.socialWork.update({ where: { id }, data: { name } });
